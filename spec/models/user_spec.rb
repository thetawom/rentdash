...
RSpec.describe User, type: :model do
  ...
  describe "#add_karma" do
    it "should update the user's amount of karma" do
      cost = 12
      user = User.new(first_name: "Frankie", last_name: "Valli")
      expect(user).to receive(:update).with(karma: user.karma + cost)
      user.add_karma(cost)
    end
  end

  describe "#deduct_karma" do
    it "should update the user's amount of karma" do
      cost = 12
      user = User.new(first_name: "Frankie", last_name: "Valli")
      expect(user).to receive(:update).with(karma: user.karma - cost)
      user.deduct_karma(cost)
    end
  end
